...
var setup_select = function(select_menu_id, options_array) {
	...
	if (Object.prototype.toString.call(options_array) == "[object Object]") {
		...
		var keys = Object.keys(options_array);
	...
	var option_len = options_array.length;
	try {
...
var get_selected_options = function(menu_id) {
	var selected = document.getElementById(menu_id).selectedOptions;
	var value_array = [];
...
var user_load = function() {
	throbber(1);
	var available_data = get_selected_options("load data list");
	setup_select("data 1", available_data);
	setup_select("data 2", available_data);
	for (year_count = 0; year_count < available_data.length; year_count++) {
...
var getPoints = function(str1, str2, loc) {
	...
	var result_loc = [];
	var arr_length = window[input_id_catalog].length;
	var tag1_selected=get_selected_options("tag1");
	for (var i = 0; i < arr_length; i++) {
		if (i > arr_length) break;
		try {
			var acc_num = window[input_id_catalog][i];
			var prop_info_obj = loc[acc_num];
			
			if (str2 == "None") {
				// first case where the second data set is not selected
				var data1 = window[input_data_file_pefix + str1];
				...
				var data1 = window[input_data_file_pefix + str1];
				var data2 = window[input_data_file_pefix + str2];
				if (data_selector(data1[acc_num], loc[acc_num], tag1_selected)) {
...
var data_selector = function(data_point, geo_data, selected_tag) {
	...
	switch (true) {
		...
		case true:
			for (tag_count = 0; tag_count < selected_tag.length; tag_count++) {
				var current_tag = data_point[tag1];
...
var execute_plot = function() {
	...
	var marker_loc = getPoints(get_selected_options("data 1")[0], get_selected_options("data 2")[0], window[input_geo_file_prefix]);
