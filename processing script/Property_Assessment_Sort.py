...
for x in reader:
    ...
    if file_line_tractker[current_year] % max_line_per_file == 0:
        ...
        output_files[current_year] = open(input_file_name + '_out/' + output_prefix + str(current_year) + '_' + str(
            math.floor(file_line_tractker[current_year] / max_line_per_file)) + '.js', 'a')
        output_files[current_year].write(output_prefix + str(current_year) + '_' + str(
            math.floor(file_line_tractker[current_year] / max_line_per_file)) + '= {')
...
for i in range(start_year, end_year + 1):
    data_list.write(
        '\'' + output_prefix + str(i) + '\' :' + str(math.floor(file_line_tractker[i] / max_line_per_file)) + ',')
